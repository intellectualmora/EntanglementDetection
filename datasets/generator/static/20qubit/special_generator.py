...
def decoder():
    ...
    h_group1 = list(map(lambda x: h1_mapping[x],h_group_))
    h_group2 = list(map(lambda x: h2_mapping[x],h_group_))
    return h_group1,h_group2,j_group


def get_Ising_ground_density_matrix(number_epoch,pid,batch_entropy,batch_ob_single_couple,batch_ob_two_couple):
    h_group1,h_group2, j_group = decoder()
    J_matrix = get_random(number_epoch*(n_qubit-1))
    ...
    for vt in range(1, number_epoch + 1):
        ...
        if vt == (number_epoch):
            tc.save(h_matrix, r'/data/phy-chely/special/h_matrix'+pid+'_'+str(n_qubit)+'.pth')
            tc.save(batch_entropy, r'/data/phy-chely/special/batch_entropy'+pid+'_'+str(n_qubit)+'.pth')
            tc.save(batch_ob_single_couple, r'/data/phy-chely/special/batch_ob_single_couple'+pid+'_'+str(n_qubit)+'.pth')
            tc.save(batch_ob_two_couple, r'/data/phy-chely/special/batch_ob_two_couple'+pid+'_'+str(n_qubit)+'.pth')
...
def main(argv):
    ...
    get_Ising_ground_density_matrix(batch_num,str(os.getpid()),*batches)
