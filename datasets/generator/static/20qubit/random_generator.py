...
device_cuda = tc.device('cpu')                  # 设备cuda设备
...
def get_Ising_ground_density_matrix(number_epoch,batch_entropy,batch_ob_single_couple,batch_ob_two_couple):
    J_matrix = get_random(number_epoch*(n_qubit-1)*9)
    for i in range(number_epoch*(n_qubit-1)*9):
        if np.random.rand() < 0.9:
            J_matrix[i] = 0
    h_matrix = get_random(number_epoch*n_qubit*3)
    for i in range(number_epoch*n_qubit*3):
        if np.random.rand() < 0.9:
            h_matrix[i] = 0
    j_group = [1,2,3,4,5,6,7,8,9]
    h_group = [10,11,12,13,14,15]
    for vt in range(1, number_epoch + 1):
        ...
        for loc in range(n_qubit-1):
            config = [para['spin']]
            for poi in range(1,16):
                if poi in j_group:
                    config.append(J_matrix[(n_qubit-1)*(vt-1)*9+9*loc+poi-1])
                elif poi in h_group and loc % 2 == 0:
                    config.append(h_matrix[(n_qubit)*(vt-1)*3+3*loc+poi-10])
                else:
                    config.append(0)
            hamilt[loc] = pm.hamiltonian_heisenberg_random(*config)
        ...
        print('epoch = ' + str(vt), str(os.getpid()))

        if vt == (number_epoch):
            tc.save(h_matrix, r'/data/phy-chely/general_30bit/h_matrix'+str(os.getpid())+'_'+str(n_qubit)+'.pth')
            tc.save(J_matrix, r'/data/phy-chely/general_30bit/j_matrix'+str(os.getpid())+'_'+str(n_qubit)+'.pth')
            tc.save(batch_entropy, r'/data/phy-chely/general_30bit/batch_entropy'+str(os.getpid())+'_'+str(n_qubit)+'.pth')
            tc.save(batch_ob_single_couple, r'/data/phy-chely/general_30bit/batch_ob_single_couple'+str(os.getpid())+'_'+str(n_qubit)+'.pth')
            tc.save(batch_ob_two_couple, r'/data/phy-chely/general_30bit/batch_ob_two_couple'+str(os.getpid())+'_'+str(n_qubit)+'.pth')
...
def main(argv):
    ...
    get_Ising_ground_density_matrix(batch_num,*batches)
